...
const modeProfiles = {
    'Ionian': KRUM_TEMPLATES['Major'],
    'Dorian': rotateArray(KRUM_TEMPLATES['Major'], 10), // Dorian starts on 2nd degree
    'Phrygian': rotateArray(KRUM_TEMPLATES['Major'], 8),
    'Lydian': rotateArray(KRUM_TEMPLATES['Major'], 5),
    'Mixolydian': rotateArray(KRUM_TEMPLATES['Major'], 7),
    'Aeolian': rotateArray(KRUM_TEMPLATES['Major'], 9),
    'Locrian': rotateArray(KRUM_TEMPLATES['Major'], 11),
};
...
const findBestMode_root = (pitchClassDistribution, root = 0) => {
    ...
    let matches = Object.entries({ ...MODE_TEMPLATES, ...KRUM_TEMPLATES }).map(([mode, profile], index) => {
        //const rotatedProfile = rotateArray(profile, root + modeRotations[index]);
        const score = pearsonCorrelation(pitchClassDistribution, profile);
        return { 'root': (root + modeRotations[index]) % 12, 'mode': mode, 'score': score }
    });

    matches = matches.sort((a, b) => b.score - a.score);
    return [matches[0], matches]; // { root: 0–11, mode: 'dorian', score: 0.85 }, all_matches
};
 ...
 * @param {*} vT 
 * @param {*} xmlDoc 
 * @returns get key and mode automatically
 */
